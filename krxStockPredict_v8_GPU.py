...
# ※ CUDA_VISIBLE_DEVICES = "-1" 제거 → GPU 활성화
#   (이전 버전에서 CPU 전용 실행으로 고정되어 있었음)

import tensorflow as tf

# ── GPU 메모리 설정 (GTX 1650 = VRAM 4GB) ─────────────────────────────────
# 필요한 만큼만 VRAM 할당 (전체 선점 방지)
_gpus = tf.config.list_physical_devices('GPU')
if _gpus:
    try:
        for _gpu in _gpus:
            tf.config.experimental.set_memory_growth(_gpu, True)
        print(f"✅ GPU 사용 가능: {len(_gpus)}개 감지됨 → {[g.name for g in _gpus]}")
    except RuntimeError as e:
        print(f"⚠️ GPU 설정 오류: {e}")
else:
    print("⚠️ GPU 미감지 → CPU로 실행됩니다")
